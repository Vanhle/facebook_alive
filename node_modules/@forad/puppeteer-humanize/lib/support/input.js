...
const getRandomChar = (char) => {
    ...
    return alpha.includes(char)
        ? alpha[utils_1.rand({ min: 0, max: alpha.length })]
        : numeric[utils_1.rand({ min: 0, max: numeric.length })];
};
...
const getProximateChar = (char) => {
    if (characters_1.isAlpha(char) || characters_1.isNumeric(char)) {
        const chars = constants_1.PROXIMATE_CHARS[char.toUpperCase()];
        if (chars === null || chars === void 0 ? void 0 : chars.length) {
            return chars[utils_1.rand({ min: 0, max: chars.length })];
        }
    }
    return undefined;
};
...
const keypressDelay = () => {
    return { delay: utils_1.rand({ min: constants_1.KEY_PRESS_MIN_DELAY, max: constants_1.KEY_PRESS_MAX_DELAY }) };
};

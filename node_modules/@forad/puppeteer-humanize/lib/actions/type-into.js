...
const typeInto = (element, text, config = {}) => tslib_1.__awaiter(void 0, void 0, void 0, function* () {
    ...
    yield element.hover();
    yield support_1.waitForTimeout({ min: 100, max: 200 });
    yield element.click({ delay: support_1.rand({ min: 5, max: 15 }) });
    yield support_1.waitForTimeout({ min: 200, max: 800 });
    // Type each character in sequence.
    let position = 0;
    for (const char of chars) {
        const charType = support_1.detectCharType(char);
        // Add potential for mistakes while typing.
        if (charType !== "termination") {
            yield type_mistake_1.typeMistake(element, char, mistakes);
        }
        // Delay slightly before punctuation.
        if (charType === "punctuation") {
            yield support_1.waitForTimeout({ min: 50, max: 100 });
        }
        // Type the correct character and add post type delay.
        // TODO: Make capital letters use shift key.
        yield element.type(char, support_1.keypressDelay());
        yield support_1.waitForTimeout(delays.all);
        ...
        if (charType &&
            ...
            yield support_1.waitForTimeout(delays[charType]);
        }
        // Vary longer delays on natural typing cadences.
        if (support_1.isCadence(position) && support_1.chance(delays.cadence.chance)) {
            yield support_1.waitForTimeout(delays.cadence);
        }
        // Increment counter for cadence tracking.
        position++;
    }
    // Pause a moment after finishing input.
    yield support_1.waitForTimeout(delays.complete);
    // Stop and return performance timer.
    timer.stop();
    support_1.debug(`Typed "${text}" over ${timer.duration()}ms.`);
    return timer;
});
